...
@TeleOp(name = "Brute Force Robot Teleop PIDF")
public class bruteForceTeleopPIDF extends LinearOpMode {
  ...
  public void runOpMode() {
    ...
    while (opModeIsActive()) {
      ...
      if (gamepad1.yWasPressed()) {
        if (shooter.curTargetVelocty == shooter.lowVelocity) {
          shooter.curTargetVelocty = shooter.highVelocity;
        } else {shooter.curTargetVelocty = shooter.lowVelocity;}
      }
...
//        shooter.F -= shooter.stepSizes[shooter.stepIndex];
//      }
//
...
//        shooter.P += shooter.stepSizes[shooter.stepIndex];
//      }
//      if(gamepad1.dpadDownWasPressed()) {
//        shooter.P -= shooter.stepSizes[shooter.stepIndex];
//      }
//      // set new PIDF coefficents
//      PIDFCoefficients pidfCoefficients = new PIDFCoefficients(shooter.P, 0, 0, shooter.F);
//      shooter.setShooterCoefficients(pidfCoefficients);

      // set velocity
//      shooter.setShooterVelocity(shooter.curTargetVelocty);

      double curVelocity = shooter.getShooterVelocity();
      double error = shooter.curTargetVelocty - curVelocity;
      telemetry.addData("target velocity", shooter.curTargetVelocty);
      telemetry.addData("current velocity", "%.2f", curVelocity);
      telemetry.addData("error", "%.2f", error);
      telemetry.addData("Tuning P", "%.4f (Dpad U/D)", shooter.P);
      telemetry.addData("Tuning F", "%.4f (D pad l/r)", shooter.F);
      telemetry.addData("Step Size", "%.4f (B Button)", shooter.stepSizes[shooter.stepIndex]);
